...
fn wait_until_ready(client: &BlockingClient, base_url: &str) {
...
async fn web_frontend_async_runner_works_inside_existing_runtime() {
    ...
    wait_until_ready_async(&client, &base_url).await;
...
async fn wait_until_ready_async(client: &Client, base_url: &str) {
    let deadline = Instant::now() + Duration::from_secs(5);
    let session_url = format!("{base_url}/api/session");

    loop {
        let outcome = match client.get(&session_url).send().await {
            Ok(response) if response.status() == StatusCode::OK => return,
            Ok(response) => format!("unexpected status: {}", response.status()),
            Err(err) => err.to_string(),
        };
